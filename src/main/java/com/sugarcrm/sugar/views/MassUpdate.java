...
public class MassUpdate extends View {
	...
	public MassUpdate(Module identityModule) throws Exception {
		...
		addControl("cancelUpdate", "a", "css", "div[data-voodoo-name='"+ identityModule.moduleNamePlural +"'] .pull-right .fld_undefined.massupdate a");
		...
		for(int i=2; i <= 21; i++){
			...
			addControl(addField, "a", "css", currentRow + " .controls.filter-actions a.btn.btn-invisible.add.pull-right");
			addControl(removeField, "a", "css", currentRow + " .controls.filter-actions a.remove.pull-right");
		}
