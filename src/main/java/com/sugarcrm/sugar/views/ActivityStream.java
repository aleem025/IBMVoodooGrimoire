...
public class ActivityStream extends View {
	...
	public void togglePreviewButton(int entryRowNum) throws Exception {
		new VoodooControl("a","css",".activitystream-list.results li:nth-of-type(" + entryRowNum + ") .actions.btn-group a[data-title='Preview']").click();
		getControl(String.format("previewButton%02d", entryRowNum)).click();
	...
	public void clickCommentButton(int entryRowNum) throws Exception {
		new VoodooControl("a","css",".activitystream-list.results li:nth-of-type(" + entryRowNum + ") .actions.btn-group a[data-original-title='Comment']").click();
		VoodooUtils.pause(500);
	}
